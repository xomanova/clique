...
function addon:Initialize()
    ...
    self.versionIsWrathClassic = WOW_PROJECT_ID == WOW_PROJECT_WRATH_CLASSIC
    ...
    if self.versionIsRelease or self.versionIsWrathClassic then
...
function addon:TalentGroupChanged()
    ...
    if addon.versionIsWrathClassic then
        currentSpec = GetActiveTalentGroup(false, false);
    end

	if self.settings.specswap and currentSpec then
        settingsKey = string.format("spec%d_profileKey", currentSpec)
        ...
        if newProfile ~= currentProfile and type(newProfile) == "string" then
            ...
            print(string.format("Clique bindings profile set to %s.", tostring(newProfile)))
        end
    end
